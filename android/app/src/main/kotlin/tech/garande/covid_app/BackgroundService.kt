...
class BackgroundService : Service(), LifecycleDetector.Listener {
    ...
    private fun startFlutterNativeView() {
        ...
        getCallbackInformation()?.let { flutterCallbackInformation ->
            flutterEngine = FlutterEngine(this).also {engine -> 
                val callbackInformation = FlutterCallbackInformation.lookupCallbackInformation(flutterCallbackInformation)

                engine.dartExecutor.executeDartCallback(DartExecutor.DartCallback(assets, FlutterMain.findAppBundlePath(), callbackInformation))
            }
        }
    }
    ...
    private fun getCallbackInformation(): Long? {
    ...
    companion object {
        private const val SHARED_PREFERENCES_NAME = "tech.garande.covid_app.BackgroundService"
